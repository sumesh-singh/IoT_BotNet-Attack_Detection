...
Implements XGBoost classifier with optimized hyperparameters for IoT botnet detection.
"""
...
class XGBoostModel:
    """XGBoost classifier optimized for IoT botnet detection."""
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.params = {**self.default_params, **self.config.get('xgboost', {})}

        logger.info(f"XGBoostModel initialized with params: {self.params}")
    ...
    def train(self, X: pd.DataFrame, y: pd.Series,
        ...
        logger.info(
            f"XGBoost training completed. Train accuracy: {train_score:.4f}")
    ...
    def explain_prediction(self, X: pd.DataFrame, sample_idx: int = 0) -> Dict[str, Any]:
        """Explain prediction for a specific sample using SHAP values if available."""
        ...
        # Get feature contributions using feature importance (simplified)
    ...
    def get_training_curves(self) -> Dict[str, List[float]]:
        """Get training curves if available from XGBoost."""

        if not self.is_trained:
            return {}

        # XGBoost stores evaluation results if eval_set was provided
...
if __name__ == '__main__':
    ...
    print(xgb_model.get_feature_importance(10))

    # Model info
    print("\nModel Info:")
    print(xgb_model.get_model_info())
